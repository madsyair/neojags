...
{
  return (TAU(par) > 0&&V(par)>0&&KAPPA(par)>0);
}
...
vector<double const *> const &par, bool give_log) const
{
  ...
  double e =  x-mu;
  double z = e/sigma;
  double logpdf1 = dt((kappa*z), v, 1);
  double logpdf2 = dt(e/(sigma*kappa), v, 1);
  ...
  const
{
  ...
  double kappa2 = kappa*kappa;
  double e =  x-mu;
  double cdf1 = 2.0*pt(kappa*e/sigma, v, 1, 0);
  double cdf2 = 1 + 2.0*kappa2*(pt(e/(sigma*kappa), v, 1, 0)-0.5);
	...
	bool log_p) const
{
  ...
  xp = (lower)? xp: 1-xp;
  double q1 = mu+(sigma/kappa)*qt(xp*(1.0+kappa2)/2, v, 1, 0);
  double q2 = mu+(sigma*kappa)*qt((xp*(1.0+kappa2)-1.0)/(2.0*kappa2) + 0.5,v, 1, 0);
