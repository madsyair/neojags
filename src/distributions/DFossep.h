...
/**
 * <pre>
 * x ~ dfskew.ep(mu, tau, nu, alpha);
 * f(x) = (c/(1/sqrt(tau)))*exp((-1/2)*pow((abs(alpha*((x-mu)/(1/sqrt(tau)))),nu)); x < mu
 * f(x) = (c/(1/sqrt(tau)))*exp((-1/2)*pow((abs(((x-mu)/(1/sqrt(tau)))/alpha),nu)); x >= mu
 * * </pre>
